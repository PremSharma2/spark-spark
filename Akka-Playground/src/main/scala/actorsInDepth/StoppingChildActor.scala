...
import actorsInDepth.StoppingChildActor.Parent.{ChildEvent, Event}

object StoppingChildActor {
  /*
      ...
      - ActorSystem creates
        - the top-level (root) guardian, with children
          - Actor-system guardian (for Akka internal messages)
  ...
  object Parent {
    ...
    case class ChildEvent(message: String) extends Event
    ...
    def idle(): Behavior[Command] = Behaviors.receive { (context, message) =>
      message match {
        case CreateChild(name) =>
          ...
          val childRef: ActorRef[Event] = context.spawn(Child(), name)
          active(childRef)
      }
    }

    def active(childRef: ActorRef[Event]): Behavior[Command] = Behaviors.receive[Command] { (context, message) =>
      message match {
        case TellChild(message) =>
          context.log.info(s"[parent] Sending message $message to child")
          childRef ! ChildEvent(message) // <- send a message to another actor
          Behaviors.same

        case StopChild =>
          context.log.info("Received StopChild Event :[parent] stopping child Actor")
          context.stop(childRef) // only works with CHILD actors
          idle() // idle state means the very beginning state i.e where it will recive Tellchild Event
  ...
  object Child {
    def apply(): Behavior[Event] = Behaviors.receive[Event] {
      (context, event) =>
      event match {
        case ChildEvent(message) =>
  ...
  object Parent_V2 {
    ...
    def active(children: Map[String, ActorRef[Event]]): Behavior[Command] = Behaviors.receive[Command] {
      (context, message) =>
      ...
      message match {
        ...
        case TellChild(name, message) =>
          ...
          childOption.fold(context.log.info(s"[parent] Child '$name' could not be found"))(child => child ! ChildEvent(message))
          Behaviors.same
