...
object InterceptingLogsSpec {
  ...
  // payment system
  /*
    checkout actor -> payment manager actor
           response(PaymentAccepted) from  payment manager actor       <-

          checkout actor         -> fulfillment manager actor
              response from  fulfillment manager actor (OrderConfirmed)    <-
   */
  type CreditCard=String
  type Item=String

  trait PaymentProtocol
  case class Checkout(item: Item, creditCard: CreditCard) extends PaymentProtocol
  case class AuthorizeCard(creditCard: CreditCard, replyTo: ActorRef[PaymentProtocol]) extends PaymentProtocol
  case object PaymentAccepted extends PaymentProtocol
  case object PaymentDeclined extends PaymentProtocol
  case class DispatchOrder(item: Item, replyTo: ActorRef[PaymentProtocol]) extends PaymentProtocol
  case object OrderConfirmed extends PaymentProtocol

  object CheckoutActor {
    def apply(): Behavior[PaymentProtocol] = Behaviors.setup {
      context =>
