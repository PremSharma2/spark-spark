...
class BlackJack (var userCredit: Int = 100) {
  ...
  def continueGame(): Boolean = {
    ...
    while (!responses.contains(input)) {
      //"Continue the game? Enter one of (Y,y,N,n): "
      println("Continue the game? Enter one of (Y,y,N,n): ")
      input = Source.stdin.getLines.next()
    }
  ...
  def didPlayerWin(deck: Deck, playerHand: Hand, dealerHand: Hand): Boolean = {
    ...
    while (noWinner) {
      ...
      while (!responses.contains(input)) {
        println("Hit or Stand? Enter one of (H,h,S,s): ")
         input=Source.stdin.getLines.next()
      }
