...
//when there is no string or no object is constructed we can assign the refrence of Null class
  ...
  // it will autoatically make that Null subclass of all classes
  val noString:String= ref
  val noPerson : Person= ref

  //3 Nil it represents Empty Collection i.e when tail is null then we can represent that
  // null refrence to Nil
  // Nil is an singleton object that can be attributed to any Empty List
